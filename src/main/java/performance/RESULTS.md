...
Tables inserted with results as nano seconds converted into a markdown form the output of the PerformanceMethods.
...
Both lists uses the simple short algorithm to put it to the end. The complexity is not tied to the list length. Although, 
...
Node, once found it just swaps the Node its pointing to the next next one. 
